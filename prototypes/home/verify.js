/* Accessibility (axe) + reduced-motion verification for the prototype. */
const { chromium } = require("playwright");
const { AxeBuilder } = require("@axe-core/playwright");

(async () => {
  const browser = await chromium.launch();
  const url = "http://127.0.0.1:8613/prototypes/home/";

  // axe on default page and on the opened pause panel
  const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page = await context.newPage();
  await page.goto(url, { waitUntil: "networkidle" });
  await page.click("#cravingBtn");
  await page.waitForTimeout(400);
  const results = await new AxeBuilder({ page }).analyze();
  console.log("axe violations:", results.violations.length);
  results.violations.forEach((v) => {
    console.log("-", v.id, "|", v.impact, "|", v.help);
    v.nodes.slice(0, 4).forEach((n) => console.log("   ", n.target.join(" ")));
  });
  await context.close();

  // Reduced motion: everything should still be visible immediately
  const rmContext = await browser.newContext({
    viewport: { width: 1440, height: 900 },
    reducedMotion: "reduce",
  });
  const rm = await rmContext.newPage();
  await rm.goto(url, { waitUntil: "networkidle" });
  const hiddenCount = await rm.evaluate(() => {
    return Array.from(document.querySelectorAll(".rise")).filter((el) => {
      const s = getComputedStyle(el);
      return parseFloat(s.opacity) < 0.99;
    }).length;
  });
  console.log("reduced-motion hidden .rise elements:", hiddenCount);
  await rm.screenshot({ path: "shot-reduced-motion.png" });
  await rmContext.close();

  await browser.close();
  console.log("verify done");
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
